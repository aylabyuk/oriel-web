      ...
      <Html center transform occlude scale={0.35}>
        ...
        <div
          style={{
            position: 'relative',
            ...(!enteredRef.current
              ? { animation: 'label-drop 0.4s ease-out both' }
              : undefined),
          }}
        >
          <div
            key={showTimer ? 'active' : 'inactive'}
            className="rounded-full p-[5px] transition-transform duration-300 select-none"
            style={
              showTimer
                ? {
          ...
          >
            <div className="flex items-center gap-1.5 rounded-full bg-neutral-900 px-2.5 py-1">
              ...
              {cardCount != null && (
                <div className="flex size-5 shrink-0 items-center justify-center rounded-full bg-white/15 text-[10px] leading-none font-bold text-white/80">
                  {cardCount}
                </div>
              )}
            </div>
          ...
          {toast && (
            <div
              ...
              className="rounded-full px-3 py-1 text-sm font-bold whitespace-nowrap text-white"
            >
              {toast.message}
            </div>
          )}
